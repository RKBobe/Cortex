...
class AIContextManager:
    ...
    def ingest_text(self, user_id: str, topic: str, content: str, filename: str):
        ...
        collection.add(embeddings=[doc_embedding], documents=[document], ids=[str(uuid.uuid4())])
    ...
    def _ingest_directory(self, user_id: str, repo_path: str, topic: str):
            ...
            for file in files:
                if any(file.endswith(ext) for ext in SOURCE_CODE_EXTENSIONS):
                    ...
                    try:
                        ...
                        metadatas.append({"topic_id": f"{user_id}-{topic}", "file": relative_path})
        ...
        embeddings = genai.embed_content(model=self.embedding_model_name, content=documents, task_type="RETRIEVAL_DOCUMENT")['embedding']
    ...
    def process_prompt(self, user_id: str, topic: str, user_prompt: str) -> str:
        ...
        if collection.count() > 0:
            ...
            results = collection.query(query_embeddings=[prompt_embedding], n_results=min(3, collection.count()))
...
"""
        ...
        collection.add(
            ...
            ids=[str(uuid.uuid4())],
            metadatas=[{"topic_id": f"{user_id}-{topic}"}] # Add metadata to conversation snippets too
        )
